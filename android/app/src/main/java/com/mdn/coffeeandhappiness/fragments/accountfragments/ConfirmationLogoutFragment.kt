...
class ConfirmationLogoutFragment : DialogFragment() {
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val alertDialog = AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Yes", null) // Null click listener for now
            .setNegativeButton("No", null) // Null click listener for now
            .create()
        ...
        alertDialog.setOnShowListener { dialog ->
            ...
            positiveButton.setTextColor(ContextCompat.getColor(requireContext(), R.color.default_green))
            negativeButton.setTextColor(ContextCompat.getColor(requireContext(), R.color.default_green))

            // Set custom text style
            val customFontStyle = R.style.ACCOUNT_MAIN_DIALOG_TEXT // Use the custom style defined in styles.xml

            // Apply custom font style to message
            (dialog.findViewById<View>(android.R.id.message) as TextView).setTextAppearance(requireContext(), customFontStyle)
            ...
            positiveButton.setOnClickListener {
                ...
                accountController.logout(requireActivity().getSharedPreferences("Account", Context.MODE_PRIVATE))
